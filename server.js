...
// ─── 安全序列化：防止破壞 HTML/JS ───────────────────
// 修正點：改用標準的特殊字元替換，百分之百避免多行字串內的 / 符號崩潰
...
'  <div class="sb" id="st">產出戰略情報中...</div>\n' +
'</div></div>\n' +
...
'<script>\n' +
'var all = JSON.parse(\'' + DATA + '\'.replace(/\\\\u003c/g, \'<\').replace(/\\\\u003e/g, \'>\'));\n' +
'var LK="lv7",BK="bk7";\n' +
...
'function hardRefresh(){document.getElementById("st").innerHTML=\'<span class="spin"></span>重新提取情報...\';fetch("/api/refresh",{method:"POST"}).then(function(r){return r.json();}).then(function(res){if(res.ok){fetch("/news.json?t="+new Date().getTime()).then(function(res2){return res2.json();}).then(function(d){if(d&&d.length){all=d;render();}});}}).catch(function(){window.location.reload();});}\n' +
'setInterval(function(){cd--;document.getElementById("cd").textContent=cd;if(cd<=0){cd=60;fetch("/news.json?t="+new Date().getTime()).then(function(res){return res.json();}).then(function(d){if(d&&d.length){all=d;render();}});}},1000);\n' +
'if(!all || !all.length){\n' +
